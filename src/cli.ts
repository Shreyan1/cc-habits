...
import {
  ...
  deregisterHooks,
  removeImportFromClaudeMd,
  uninstallLocalGitHook,
  uninstallGlobalGitTemplateHook,
  deregisterJsonHooks,
  deregisterKimiHooks,
  deregisterClineHooks,
  areHooksRegistered,
  hookProofPaths,
  readRegisteredHooks,
  installPaths,
...
import {
  ...
  renderBrandedCard,
  renderHabitLine,
  printMemoriesEmptyState,
  renderMemoryLine,
  promptChoice,
  promptYesNo,
  promptYesNoDefaultTrue,
  promptSecret,
  withSpinner,
  steppedProgress,
  normaliseLanguages,
...
import {
  configureProvider,
  interactiveOllamaSetup,
  showProviderMenu,
  OLLAMA_DEFAULT_MODEL,
  validateProviderFlag,
  reconfigureProviderMenu,
} from "./cli-provider";
  ...
  try {
    for (const file of hookProofPaths(toolId, settingsPath)) {
      ...
      process.stdout.write(
        c(DIM, `    ↳ proof, written to ${tildePath(file)}:\n`),
      );
      for (const { event, command } of hooks) {
        process.stdout.write(
          c(DIM, `        ${(event + ":").padEnd(17)} ${command}\n`),
        );
      }
    }
  } catch {
...
function printAntigravityNotice(): void {
  process.stdout.write(
    c(
      YELLOW,
      "  ! Google has moved the Gemini CLI to its new Antigravity CLI (agy),\n",
    ),
  );
  process.stdout.write(
    c(
      DIM,
      "    retiring the old Gemini CLI on 2026-06-18 and relocating hooks into a\n",
    ),
  );
  process.stdout.write(
    c(DIM, "    new format it has not finalized publicly yet.\n"),
  );
  process.stdout.write(
    c(
      GREEN,
      "    Good news: your learned habits still inject into Antigravity through\n",
    ),
  );
  process.stdout.write(
    c(GREEN, "    GEMINI.md, so nothing you have learned is lost.\n"),
  );
  process.stdout.write(
    c(
      DIM,
      "    We are holding off on Antigravity capture hooks only until Google\n",
    ),
  );
  process.stdout.write(
    c(
      DIM,
      "    confirms the format, so we never register a hook that quietly fails.\n",
    ),
  );
  process.stdout.write(
    c(
      DIM,
      "    Sorry for the gap this migration introduced; we are tracking it and\n",
    ),
  );
  process.stdout.write(
    c(
      DIM,
      "    will ship Antigravity capture the moment the format is public.\n",
    ),
  );
}
...
function renderRecommendedPlan(detected: { name: string }[]): void {
  ...
  process.stdout.write(
    `    ${tick} register hooks in: ${c(BOLD, toolNames)}\n`,
  );
  process.stdout.write(
    `    ${tick} keep your current AI provider if one is configured\n`,
  );
  process.stdout.write(
    `    ${tick} bootstrap habits from past sessions (if any)\n`,
  );
  process.stdout.write(
    `    ${tick} install the local git capture hook in this repo\n`,
  );
  process.stdout.write(`    ${tick} scan this repo into its .cch/ store\n`);
  process.stdout.write(`    ${tick} enable memory learning\n`);
  process.stdout.write(
    `    ${dash} leave the system-wide git hook off (enable later with manual setup)\n`,
  );
  process.stdout.write("\n");
}
...
export async function cmdInit(providerFlag?: string): Promise<number> {
  ...
  if (!consentGiven()) {
    ...
    const agreed = await promptYesNoDefaultTrue(
  ...
  if (process.stdin.isTTY) {
    const mode = await runSelectMenu(
      c(BOLD, "  How would you like to set up cc-habits?"),
      [
        {
          label: "Recommended Initialisation   set up with the safe defaults",
          value: "rec",
        },
        {
          label: "Manual Configuration         choose each step yourself",
          value: "man",
        },
      ],
    );
  ...
  const askYes = (q: string): Promise<boolean> =>
    recommended
      ? (process.stdout.write(
          q + c(GREEN, "Yes") + c(DIM, " (recommended)") + "\n",
        ),
  ...
  if (detected.length > 0) {
    ...
    for (const tool of detected) {
      ...
      if (tool.id === 'claude-code') {
        ...
        if (register) {
          process.stdout.write("\n");
          const { postAdded, stopAdded, promptAdded, sessionStartAdded } =
            registerHooks(hookBin);
          process.stdout.write(
            `    ${postAdded ? tick : dash} PostToolUse hook ${postAdded ? "registered" : "already registered"}\n`,
          );
          process.stdout.write(
            `    ${stopAdded ? tick : dash} Stop hook ${stopAdded ? "registered" : "already registered"}\n`,
          );
          process.stdout.write(
            `    ${promptAdded ? tick : dash} UserPromptSubmit hook ${promptAdded ? "registered" : "already registered"}\n`,
          );
          process.stdout.write(
        ...
        try {
          ...
          const suffix = register
            ? ""
            : c(DIM, " (so Claude reads your learned habits)");
          process.stdout.write(
            `    ${importAdded ? tick : dash} preferences.md import ${importAdded ? "added to" : "already in"} ~/.claude/CLAUDE.md${suffix}\n`,
          );
        } catch (e) {
          process.stdout.write(
            c(
              YELLOW,
              `    ! could not wire preferences.md import: ${String(e).slice(0, 60)}\n`,
            ),
          );
        }
        ...
        if (isAntigravityMigrated()) {
          ...
          process.stdout.write(
            `    ${tick} habits will inject into Antigravity via GEMINI.md\n`,
          );
        } else {
          const register = await askYes(
            "  Register hooks in Gemini CLI? [Y/n] ",
          );
          if (register) {
            process.stdout.write("\n");
            const { postAdded, stopAdded, promptAdded, sessionStartAdded } =
              registerJsonHooks(tool.settingsPath, "gemini", hookBin);
            process.stdout.write(
              `    ${postAdded ? tick : dash} AfterTool hook ${postAdded ? "registered" : "already registered"}\n`,
            );
            process.stdout.write(
              `    ${stopAdded ? tick : dash} AfterAgent hook ${stopAdded ? "registered" : "already registered"}\n`,
            );
            process.stdout.write(
              `    ${promptAdded ? tick : dash} BeforeAgent hook ${promptAdded ? "registered" : "already registered"}\n`,
            );
            process.stdout.write(
              `    ${sessionStartAdded ? tick : dash} SessionStart hook ${sessionStartAdded ? "registered" : "already registered"}\n`,
            );
        ...
        if (register) {
          ...
          process.stdout.write("\n");
          const { postAdded, promptAdded } = registerCodexHooks(
            tool.settingsPath,
            hookBin,
          );
          process.stdout.write(
            `    ${postAdded ? tick : dash} PostToolUse hook (captures edits) ${postAdded ? "registered" : "already registered"}\n`,
          );
          process.stdout.write(
            `    ${promptAdded ? tick : dash} UserPromptSubmit hook (compiles habits) ${promptAdded ? "registered" : "already registered"}\n`,
          );
          ...
          process.stdout.write("\n");
          process.stdout.write(
            c(YELLOW, "    ! Codex disables new hooks until you trust them.\n"),
          );
          process.stdout.write(
            c(
              DIM,
              '      Open Codex: it will prompt that "hooks are new or changed",\n',
            ),
          );
          process.stdout.write(
            c(
              DIM,
              "      approve the cc-habits hooks (or enable them in Codex's Hooks view).\n",
            ),
          );
          process.stdout.write(
            c(
              DIM,
              "      Until then Codex captures nothing; `cch bootstrap` still learns from past sessions.\n",
            ),
          );
          ...
          process.stdout.write(
            c(
              DIM,
              "      Note: Codex often edits via shell (sed/perl) the hook cannot see.\n",
            ),
          );
          process.stdout.write(
            c(
              DIM,
              "      Enabling git capture below ensures those edits are still recorded.\n",
            ),
          );
        }
      } else if (tool.id === "kimi") {
        const register = await askYes(
          "  Register hooks in Kimi Code CLI? [Y/n] ",
        );
        if (register) {
          process.stdout.write("\n");
          const { postAdded, stopAdded, promptAdded, sessionStartAdded } =
            registerKimiHooks(tool.settingsPath, hookBin);
          process.stdout.write(
            `    ${postAdded ? tick : dash} PostToolUse hook ${postAdded ? "registered" : "already registered"}\n`,
          );
          process.stdout.write(
        ...
        const register = await askYes(
          "  Register hooks in Cline/RooCode? [Y/n] ",
        );
        if (register) {
          process.stdout.write("\n");
          const { postAdded, stopAdded } = registerClineHooks(
            tool.settingsPath,
            hookBin,
          );
          process.stdout.write(
        ...
        process.stdout.write(
          `  ${dash} Cursor has no hooks; enable Git capture below to learn from its edits.\n`,
        );
      }
    }
    ...
    if (syncTargetsToEnable.size > 0) {
      ...
      const files = [...syncTargetsToEnable]
        .map((t) => fileFor[t] ?? t)
        .join(", ");
      process.stdout.write(
        `  ${tick} Auto-sync enabled: learned habits will refresh ${files} after each session.\n`,
      );
    }
    process.stdout.write("\n");
  } else {
    ...
    const importAdded = addImportToClaudeMd();
    process.stdout.write(
      `  ${importAdded ? tick : dash} preferences.md import ${importAdded ? "added to" : "already in"} ~/.claude/CLAUDE.md\n`,
    );
  ...
  if (providerFlag) {
    ...
    if (recommended) {
      process.stdout.write(
        `  ${tick} Keeping current provider: ${c(BOLD, resolveProviderLabel())}\n`,
      );
    } else {
  ...
  if (!providerReady) {
    ...
    process.stdout.write(
      c(
        DIM,
        "  cc-habits is capturing your edits now, but it needs an AI provider to turn them into habits.\n",
      ),
    );
    process.stdout.write(
      c(
        DIM,
        "  Add one any time:  cch init --provider anthropic   (API key)\n",
      ),
    );
    process.stdout.write(
      c(
        DIM,
        "  No key? Run a free local model:  https://ollama.com/download   then  cch init --provider ollama\n",
      ),
    );
  }
  const habitsEmpty = parseHabits(readHabitsMd());
  const hasExistingHabits = Object.values(habitsEmpty).some(
    (h) => h.length > 0,
  );
        ...
        try {
          ...
          if (result.habitsLearned > 0) {
            ...
            process.stdout.write(
              c(
                DIM,
                "  No clear patterns found yet. Habits will emerge as you code more.\n",
              ),
            );
          }
        } catch (e) {
          process.stdout.write(
            c(DIM, `  Bootstrap skipped: ${String(e).slice(0, 80)}\n`),
          );
        }
      }
    }
  ...
  if (inGitRepo) {
    ...
    if (codexRegistered) {
      process.stdout.write(
        c(
          DIM,
          "  Recommended for Codex: git capture records edits the hook misses (shell/sed/perl).\n",
        ),
      );
    }
    const installLocal =
      codexRegistered || recommended
        ? await askYes(
            "  Install git capture hook locally in this project? [Y/n] ",
          )
        : await promptYesNo(
            "  Install git capture hook locally in this project? [y/N] ",
          );
    if (installLocal) {
      ...
      const gitMark =
        added === "installed" ? tick : added === "already" ? dash : "!";
      const gitMsg =
        added === "installed"
          ? "installed"
          : added === "already"
            ...
            : "failed to install";
      process.stdout.write(
        `  ${gitMark} Local Git post-commit hook ${gitMsg}\n`,
      );
    }
    process.stdout.write("\n");
  }

  const globalHookFile = path.join(
    os.homedir(),
    ".git-templates",
    "hooks",
    "post-commit",
  ...
  try {
    ...
    globalHookAlready =
      ghContent.includes("cc-habits git-capture") ||
      ghContent.includes("cch git-capture");
  } catch {
    /* not installed yet */
  }
  ...
  if (globalHookAlready) {
    process.stdout.write(
      `  ${dash} Global Git template post-commit hook already installed\n`,
    );
  } else {
    const installGlobal = await askNo(
      "  Install git capture hook globally for all new repositories? [y/N] ",
    );
    if (installGlobal) {
      ...
      const gitMsg =
        added === "installed"
          ...
          : added === "already"
            ...
            : "failed to install";
      process.stdout.write(
        `  ${gitMark} Global Git template post-commit hook ${gitMsg}\n`,
      );
    }
  }
  process.stdout.write("\n");
  ...
  if (providerReady) {
    ...
    process.stdout.write(
      c(
        DIM,
        "  Scanning this repository for habits (into its .cch/ store)...\n",
      ),
    );
    try {
      ...
      process.stdout.write(
        c(DIM, "  Repo scan skipped. Run `cch learn --repo` to retry.\n"),
      );
    }
  }
  ...
  process.stdout.write("\n\n");
  process.stdout.write(
    c(BOLD, "cc-habits is ready.") +
      " Start a coding session or commit changes to begin learning.\n",
  );
  process.stdout.write(
    c(
      DIM,
      "  To verify: run `cch status` after your first session. It shows when each tool last fired the hook,\n",
    ) +
      c(
        DIM,
        '  logged only when the tool itself runs it (liveness proof, not just "registered").\n',
      ),
  );
  return 0;
}
...
function renderScopeBanner(s: ResolvedViewScope): void {
  ...
  if (s.scope === "repo") {
    ...
    process.stdout.write(
      c(DIM, "  scope: ") +
        c(BOLD, "global") +
        c(DIM, " (~/.cc-habits)  ·  this repo also has a local store: ") +
        c(CYAN, "cch view --repo") +
        "\n\n",
    );
  }
}
...
export function renderHabitsView(
  ...
  const activeHabits = Object.values(cats)
    .flat()
    .filter((h) => (h.sessions_seen ?? 1) >= 2).length;
  ...
  const statusDisplay = isGloballyDisabled()
    ? "Disabled"
    : `Active  ·  ${providerDisplay}`;
  ...
  const resolvedSessionId =
    activeSessionId ||
    (allSignals.length > 0
      ? allSignals[allSignals.length - 1].session_id
      : undefined);
  if (resolvedSessionId) {
    const sessionSignals = allSignals.filter(
      (s) => s.session_id === resolvedSessionId,
    );
    ...
    if (
      ...
      process.stdout.write(
        `    Signals:    ${c(BOLD, String(sessionSignals.length))} captured\n`,
      );
      if (sessionCreated.length > 0 || sessionReinforced.length > 0) {
        ...
        for (const h of sessionReinforced) {
          process.stdout.write(
            `      ${c(CYAN, "^")} ${h.rule} (confidence: ${Math.round(h.confidence * 100)}%)\n`,
          );
        }
      } else {
  ...
  if (totalHabits === 0) {
    ...
    process.stdout.write(
      c(DIM, "  Use Claude Code for a session, then check back.\n"),
    );
  } else {
    ...
    const memSuffix =
      memoriesEnabled() && totalMemories > 0
    ...
    process.stdout.write(
      `  ${c(BOLD, String(activeHabits))} active ${hw} across ` +
        ...
        c(
          DIM,
          `${learningHabits} learning  ·  ${totalSignals} ${sw} processed`,
        ) +
        c(DIM, memSuffix) +
        "\n",
    );
    if (learningHabits > 0) {
      process.stdout.write(
        c(DIM, "  (learning habits activate after 1 more session)\n"),
      );
    }

    // Surface the languages cc-habits has observed (data already captured per
    // habit). Doubles as discovery for the `--lang` filter.
    const observedLangs = normaliseLanguages(
      Object.values(cats)
        .flat()
        .flatMap((h) => h.languages ?? []),
    ).sort();
    if (observedLangs.length > 0) {
      const hint = langF ? "" : "  ·  filter with `cch view --lang <lang>`";
      process.stdout.write(
        c(DIM, `  languages: ${observedLangs.join(", ")}${hint}\n`),
      );
    }
    if (langF) {
      process.stdout.write(
        c(CYAN, `  showing habits tagged `) + c(BOLD, langF) + "\n",
      );
    }
    process.stdout.write("\n");
    ...
    for (const category of Object.keys(cats).sort()) {
      ...
      const active = habits.filter(
        (h) => (h.sessions_seen ?? 1) >= 2 && matchesLang(h),
      );
      const learning = habits.filter(
        (h) => (h.sessions_seen ?? 1) < 2 && matchesLang(h),
      );
      ...
      process.stdout.write(
        c(BOLD, `  ── ${category} `) +
          c(DIM, "─".repeat(Math.max(0, 46 - category.length))) +
          "\n",
      );
    ...
    if (langF && shown === 0) {
      process.stdout.write(
        c(DIM, `  No habits tagged `) +
          c(BOLD, langF) +
          c(DIM, ".") +
          (observedLangs.length > 0
            ? c(DIM, ` Observed: ${observedLangs.join(", ")}.`)
            : "") +
          "\n\n",
      );
    }
  ...
  if (allSignals.length > 0) {
    process.stdout.write(
      c(BOLD, "  ── Recent signals ") + c(DIM, "─".repeat(30)) + "\n",
    );
    process.stdout.write("\n");
    ...
    process.stdout.write(
      c(DIM, "  No signals yet. Make some edits in Claude Code.\n"),
    );
    process.stdout.write("\n");
  }
...
function renderMemoriesSummary(ctx?: StorageContext): void {
  ...
  process.stdout.write(
    c(BOLD, "  ── Memories ") + c(DIM, "─".repeat(36)) + "\n",
  );
  if (active.length === 0) {
    process.stdout.write(
      c(
        DIM,
        `  ${candidates} candidate${candidates === 1 ? "" : "s"} still learning. See all: cch view memories\n\n`,
      ),
    );
    return;
  }
  ...
  if (candidates > 0) {
    process.stdout.write(
      c(
        DIM,
        `  +${candidates} candidate${candidates === 1 ? "" : "s"} learning · full list: cch view memories\n`,
      ),
    );
  }
  process.stdout.write("\n");
...
export async function cmdViewInteractive(): Promise<number> {
  ...
  const items: { label: string; value: string }[] = [
    {
      label: "Habits + memories   the full picture (global store)",
      value: "all",
    },
    {
      label: "Habits only         grouped, with confidence and languages",
      value: "habits",
    },
    {
      label: "Memories            project cautions and corrections",
      value: "memories",
    },
    {
      label: "Preferences         the file your agents actually read",
      value: "prefs",
    },
  ];
  ...
  if (resolveViewScope("repo").scope === "repo") {
    items.push({
      label: "This repo (.cch/)   habits scoped to this repository",
      value: "repo",
    });
  }
...
export function cmdPrefs(requestedScope?: ViewScope): number {
  const scope = resolveViewScope(requestedScope);
  const prefsPath = scope.ctx
    ? scope.ctx.preferencesFile
    : storagePaths.preferencesFile;
  const subtitle =
    scope.scope === "repo"
  ...
  if (!fs.existsSync(prefsPath)) {
    ...
    process.stdout.write(
      c(
        DIM,
        "  Run `cch init` to set up hooks, then code a few sessions for habits to appear.\n\n",
      ),
    );
    return 0;
  }
  ...
  if (!content) {
    process.stdout.write(
      c(DIM, "  preferences.md is empty, no active habits yet.\n"),
    );
    process.stdout.write(
      c(
        DIM,
        "  Habits appear after 2+ coding sessions where you correct the agent.\n\n",
      ),
    );
    return 0;
  }
...
export function cmdMemoriesDelete(text: string): number {
  if (!text.trim()) {
    process.stderr.write(
      "cc-habits memories --delete: requires a memory text string.\n",
    );
    process.stderr.write(
      '  Usage: cc-habits memories --delete "memory text"\n',
    );
    return 1;
  }
  initMemoriesMd();
  ...
  if (/^cch[a-f0-9]{8}$/i.test(targetText)) {
    ...
    for (const sectionMemories of Object.values(sections)) {
      const match = sectionMemories.find(
        (m) => getRuleHash(m.text) === targetText.toLowerCase(),
      );
      if (match) {
    ...
    if (!foundHash) {
      process.stderr.write(
        `  Error: memory hash ${text} not found in memories.md\n`,
      );
      return 1;
    }
  }

  const normalised = targetText
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9\s]/g, "")
    .replace(/\s+/g, " ");
  ...
  const matchNormalised = (
    ...
    return (
      mText
        ...
        .toLowerCase()
        .replace(/[^a-z0-9\s]/g, "")
        .replace(/\s+/g, " ") === targetNormalised
    );
  };

  for (const sectionMemories of Object.values(sections)) {
    const idx = sectionMemories.findIndex((m) =>
      matchNormalised(m.text, normalised),
    );
    if (idx >= 0) {
  ...
  if (!found && normalised.endsWith(" candidate")) {
    const fallbackNormalised = normalised
      .slice(0, normalised.length - " candidate".length)
      .trim();
    for (const sectionMemories of Object.values(sections)) {
      const idx = sectionMemories.findIndex((m) =>
        matchNormalised(m.text, fallbackNormalised),
      );
      if (idx >= 0) {
        foundText = sectionMemories[idx].text;
        sectionMemories.splice(idx, 1);
  ...
  if (!found && targetText.trim().toLowerCase().endsWith(" (candidate)")) {
    tombstoneText = targetText
      .trim()
      .slice(0, targetText.trim().length - " (candidate)".length)
      .trim();
  }
  ...
  if (found) {
    ...
    process.stdout.write(
      `  tombstoned (not found in memories.md): ${tombstoneText}\n`,
    );
  }
  return 0;
}
...
export async function cmdMemoriesToggle(enabled: boolean): Promise<number> {
  ...
  if (enabled) {
    ...
    process.stdout.write(
      c(
        DIM,
        "  Future sessions will learn from corrections you make to agent output.\n",
      ),
    );
    process.stdout.write(
      c(
        DIM,
        "  Next: keep coding, then run `cch memories` to see what was learned.\n",
      ),
    );
  } else {
    process.stdout.write(
      c(DIM, "  ~ Memory learning off. Existing memories are kept.\n"),
    );
  }
  return 0;
}
...
export async function cmdMemories(requestedScope?: ViewScope): Promise<number> {
  ...
  process.stdout.write("\n");
  process.stdout.write(
    c(BOLD + CYAN, "  cc-habits") + c(BOLD, " · coding memories\n"),
  );
  process.stdout.write(
    c(
      DIM,
      `  ${scope.ctx ? scope.ctx.memoriesFile : storagePaths.memoriesFile}\n`,
    ),
  );
  process.stdout.write("\n");
  renderScopeBanner(scope);
  ...
  process.stdout.write(
    c(
      DIM,
      `  To delete: cch memories --delete <id>  (the id in brackets, without the brackets)\n\n`,
    ),
  );
  ...
  for (const section of Object.keys(sections).sort()) {
    ...
    process.stdout.write(
      c(BOLD, `  ── ${section} `) +
        c(DIM, "─".repeat(Math.max(0, 46 - section.length))) +
        "\n",
    );
    for (const memory of memories.filter((m) => (m.sessions_seen ?? 1) >= 2))
      renderMemoryLine(memory, false);
    for (const memory of memories.filter((m) => (m.sessions_seen ?? 1) < 2))
      renderMemoryLine(memory, true);
    process.stdout.write("\n");
  }
...
export function cmdLog(limit = 20): number {
  ...
  process.stdout.write("\n");
  process.stdout.write(
    c(BOLD + CYAN, "  cc-habits") + c(BOLD, " · capture log\n"),
  );
  process.stdout.write(c(DIM, `  ${storagePaths.logFile}\n`));
  process.stdout.write(
    c(
      DIM,
      "  Diffs are redacted (email / PAN / card) at capture. Run `cc-habits reset --yes` to erase.\n",
    ),
  );
  process.stdout.write("\n");
  ...
  process.stdout.write(
    c(
      DIM,
      `  showing ${recent.length} of ${signals.length} captured signal${signals.length === 1 ? "" : "s"}\n\n`,
    ),
  );
  for (const sig of recent) {
    ...
    process.stdout.write(
      `  ${c(DIM, ts)}  ${c(CYAN, term(sig.file ?? ""))}${lang}\n`,
    );
    const diffLines = (sig.diff ?? "")
      .split("\n")
      .filter(
    ...
    for (const ln of diffLines.slice(0, 4)) {
      if (ln.startsWith("+"))
        process.stdout.write(`    ${c(GREEN, term(ln.slice(0, 80)))}\n`);
...
export function cmdReset(yes: boolean): number {
  if (!yes) {
    process.stderr.write(
      "cc-habits reset: requires --yes flag to confirm deletion.\n",
    );
    return 1;
  }
  ...
  process.stdout.write(
    "cc-habits: reset complete. Hooks and tombstones remain.\n",
  );
  return 0;
}
...
export async function cmdUninstall(yes: boolean): Promise<number> {
  if (!yes) {
    const confirm = await promptYesNo(
      "  Are you sure you want to completely uninstall cc-habits and delete all learned data? [y/N] ",
    );
    if (!confirm) {
  ...
  try {
    const { postRemoved, stopRemoved, promptRemoved, sessionStartRemoved } =
      deregisterHooks();
    if (postRemoved || stopRemoved || promptRemoved || sessionStartRemoved) {
      process.stdout.write(
        `  ${tick} Removed hooks from Claude Code (~/.claude/settings.json)\n`,
      );
    }
  } catch (e) {
    process.stdout.write(
      `  ${dash} Claude Code hooks clean skipped: ${String(e)}\n`,
    );
  }
  ...
  try {
    if (removeImportFromClaudeMd()) {
      process.stdout.write(
        `  ${tick} Removed @import from ~/.claude/CLAUDE.md\n`,
      );
    }
  } catch (e) {
    process.stdout.write(
      `  ${dash} CLAUDE.md import clean skipped: ${String(e)}\n`,
    );
  }
    ...
    for (const tool of detected) {
      try {
        if (tool.id === "gemini") {
          ...
          if (postAdded)
            process.stdout.write(
              `  ${tick} Removed hooks from Gemini CLI (${tool.settingsPath})\n`,
            );
        } else if (tool.id === "codex") {
          const jsonFile = path.join(
            path.dirname(tool.settingsPath),
            "hooks.json",
          );
          const { postAdded } = deregisterJsonHooks(jsonFile);
          if (postAdded)
            process.stdout.write(
              `  ${tick} Removed hooks from Codex CLI (${jsonFile})\n`,
            );
        } else if (tool.id === "kimi") {
          if (deregisterKimiHooks(tool.settingsPath)) {
            process.stdout.write(
              `  ${tick} Removed hooks from Kimi Code CLI (${tool.settingsPath})\n`,
            );
          }
        } else if (tool.id === "cline") {
          if (deregisterClineHooks(tool.settingsPath)) {
            process.stdout.write(
              `  ${tick} Removed hooks from Cline/RooCode (${tool.settingsPath})\n`,
            );
          }
        }
      } catch (e) {
        process.stdout.write(
          `  ${dash} Clean hooks for ${tool.name} skipped: ${String(e)}\n`,
        );
      }
    }
  } catch {
  ...
  try {
    ...
    process.stdout.write(
      `  ${dash} Local Git hook clean skipped: ${String(e)}\n`,
    );
  }
  ...
  try {
    if (uninstallGlobalGitTemplateHook()) {
      process.stdout.write(
        `  ${tick} Removed global Git template capture hook\n`,
      );
    }
  } catch (e) {
    process.stdout.write(
      `  ${dash} Global Git hook clean skipped: ${String(e)}\n`,
    );
  }
...
export function lastFiredBySource(
  signals: ReadonlyArray<{ ts: string; source?: string; file: string }>,
): Record<string, FiredInfo> {
...
export function readExtractionHealth(): ExtractionHealth {
  ...
  try {
    ...
    /* history unreadable: treat as no known success */
  }
  ...
  try {
    ...
    if (fs.existsSync(errPath)) {
      ...
      for (let i = lines.length - 1; i >= 0; i--) {
        ...
        if (/memory extraction failed|auto-sync failed|sync failed/.test(msg))
          continue;
        lastFailureTs = m[1];
        lastFailureMsg = msg
          .replace(/^(?:stop|learn):\s+/, "")
          .replace(/^Error:\s+/, "")
          .slice(0, 42);
        break;
      }
    ...
    /* error.log unreadable: treat as no known failure */
  }

  const failing =
    !!lastFailureTs && (!lastSuccessTs || lastFailureTs > lastSuccessTs);
...
export function cmdStatus(proof = false): number {
  ...
  const rule = (l: string, r: string): string =>
    `${l}${"─".repeat(W - 2)}${r}\n`;
  ...
  let allHabits: Habit[] = [];
  try {
    allHabits = Object.values(parseHabits(readHabitsMd())).flat();
  } catch {
    allHabits = [];
  }
  const activeCount = allHabits.filter(
    (h) => (h.sessions_seen ?? 1) >= 2,
  ).length;
  const learningCount = allHabits.filter(
    (h) => (h.sessions_seen ?? 1) < 2,
  ).length;
  ...
  const sessionId =
    ...
    (allSignals.length > 0
      ? allSignals[allSignals.length - 1].session_id
      : undefined);
  const sessionSigs = sessionId
    ? allSignals.filter((s) => s.session_id === sessionId).length
    : 0;
      ...
      try {
        if (tool.id === "cursor") {
          ...
          const desc = cursorFired
            ? c(GREEN, "live") +
              c(
                DIM,
                ` · ${formatTimeAgo(cursorFired.ts)} · ${term(path.basename(cursorFired.file))} · ${cursorFired.count} sig${cursorFired.count === 1 ? "" : "s"}`,
              )
            : c(DIM, "git capture on commit");
          hookRows.push(
            line(`${git}  ${pad(c(BOLD, "Cursor"), NAMEW)}  ${desc}`),
          );
          continue;
        }
        const files = hookProofPaths(tool.id, tool.settingsPath);
        const entries = files.flatMap((f) =>
          readRegisteredHooks(f).map((h) => ({ ...h, file: f })),
        );
        const registered =
          tool.id === "claude-code" ? areHooksRegistered() : entries.length > 0;
        if (registered) anyRegistered = true;
        else anyUnregistered = true;
        ...
        if (!registered) {
          ...
          desc =
            ...
            c(
              DIM,
              ` · ${formatTimeAgo(fired.ts)} · ${term(path.basename(fired.file))} · ${fired.count} sig${fired.count === 1 ? "" : "s"}`,
            );
        } else if (tool.id === "codex") {
          ...
          desc =
            c(DIM, "registered") + c(YELLOW, " · shell edits not captured");
        } else {
          desc =
            c(DIM, "registered") +
            c(YELLOW, ` (edit in ${tool.name} to confirm)`);
        }
        hookRows.push(
          line(`${glyph}  ${pad(c(BOLD, tool.name), NAMEW)}  ${desc}`),
        );
        if (proof && registered && entries.length > 0) {
          ...
          for (const file of where) {
            ...
            for (const e of entries.filter((x) => x.file === file)) {
              hookRows.push(
                line(
                  c(DIM, `      ${(e.event + ":").padEnd(17)} ${e.command}`),
                ),
              );
            }
          }
        ...
        hookRows.push(
          line(
            `${git}  ${pad(c(BOLD, tool.name), NAMEW)}  ${c(DIM, "(could not read settings)")}`,
          ),
        );
      }
    }
  ...
  if (providerUsable) {
    ...
    providerVal =
      c(BOLD, resolveProviderLabel()) +
      (envKeyOnly ? c(DIM, "  (ANTHROPIC_API_KEY env)") : "");
  } else if (configExists && provider && !isParkedProvider(provider)) {
    providerVal =
      c(BOLD, provider) + c(YELLOW, "  configured, no API key found");
  } else {
    providerVal =
      c(YELLOW, "No provider configured") +
      c(DIM, "  extraction paused, capture continues");
  }
  ...
  const cloudOllamaActive =
    provider === "ollama" && isCloudOllamaModel(getConfigValue("ollama_model"));
  ...
  const habitsVal =
    activeCount === 0 && learningCount === 0
      ? c(DIM, "no habits yet")
      : c(BOLD, String(activeCount)) +
        " active" +
        (learningCount > 0 ? c(DIM, ` · ${learningCount} learning`) : "");

  const signalsVal =
    allSignals.length === 0
      ? c(DIM, "0 captured")
      : c(BOLD, String(allSignals.length)) +
        " total" +
        (sessionId ? c(DIM, ` · ${sessionSigs} this session`) : "");
  ...
  try {
    ...
    if (root) {
      ...
      if (fs.existsSync(rctx.habitsFile)) {
        ...
        const lastTs =
          rHist.length > 0 ? rHist[rHist.length - 1].ts : undefined;
        repoVal =
          c(BOLD, String(rCount)) +
          ` habit${rCount === 1 ? "" : "s"}` +
          (lastTs
            ? c(DIM, ` · learned ${formatTimeAgo(lastTs)}`)
            : c(DIM, " · run `cch learn` to populate"));
      }
    }
  } catch {
    /* repo row is best-effort */
  }
  ...
  if (health.failing) {
    extractionVal =
      ...
      c(
        YELLOW,
        `  ${repoVal ? "global · " : ""}last attempt failed ${formatTimeAgo(health.lastFailureTs!)}`,
      ) +
      (health.lastFailureMsg ? c(DIM, ` (${health.lastFailureMsg})`) : "");
  } else if (health.lastSuccessTs) {
    extractionVal =
      ok +
      c(DIM, `  ${scope}last learned ${formatTimeAgo(health.lastSuccessTs)}`);
  } else {
  ...
  if (cloudOllamaActive) {
    process.stdout.write(
      c(
        YELLOW,
        `→ Ollama model ${getConfigValue("ollama_model")} is a cloud model: redacted diffs leave your machine `,
      ) +
        c(
          DIM,
          "(not local/air-gapped). Pick a model without the `-cloud` suffix for a fully local setup.\n",
        ),
    );
  }
  ...
  try {
    ...
    const active = Object.values(cats)
      .flat()
      .filter((h) => (h.sessions_seen ?? 1) >= 2).length;
    if (!providerUsable) {
      process.stdout.write(
        c(DIM, "→ Run `cch init` to configure a provider.\n"),
      );
    } else if (allSignals.length === 0) {
      process.stdout.write(
        c(DIM, "→ Run `cch bootstrap` to seed habits from past sessions.\n"),
      );
      ...
      process.stdout.write(
        c(YELLOW, "→ Extraction is failing. ") +
          c(
            DIM,
            `Last error: ${health.lastFailureMsg ?? "see `cch log`"}. Capture continues; run \`cch learn\` to retry.\n`,
          ),
      );
    } else if (active === 0) {
      process.stdout.write(
        c(DIM, "  Keep coding, habits graduate after 2+ sessions.\n"),
      );
      ...
      process.stdout.write(
        c(YELLOW, "→ Habits are not injected: ") +
          c(
            DIM,
            "no cc-habits @import in CLAUDE.md. Run `cch init` to wire it up.\n",
          ),
      );
      ...
      process.stdout.write(
        c(YELLOW, "→ No tool hooks registered: ") +
          c(
            DIM,
            "nothing is being captured. Run `cch init` to register your tools.\n",
          ),
      );
      ...
      process.stdout.write(
        c(GREEN, "  All good.") +
          c(DIM, " Start a coding session to keep learning.\n"),
      );
    }
  } catch {
...
export function cmdShellInit(): number {
  ...
  try {
    ...
    /* fall back to PATH-resolved cc-habits */
  }
...
export function cmdTools(): number {
  ...
  for (const tool of SUPPORTED_TOOLS) {
    ...
    if (tool.id === "gemini" && antigravity) {
      process.stdout.write(
        c(
          YELLOW,
          `      note:    Google moved Gemini CLI to Antigravity (agy). Injection via\n`,
        ),
      );
      process.stdout.write(
        c(
          YELLOW,
          `               GEMINI.md still works; capture is paused until Google\n`,
        ),
      );
      process.stdout.write(
        c(YELLOW, `               publishes the new hook format.\n`),
      );
    }
  }
  process.stdout.write(
  ...
  process.stdout.write(
    c(DIM, "  Run `cch init` to register hooks for your detected tools.\n\n"),
  );
  return 0;
}
...
export function cmdTombstone(rule: string): number {
  ...
  if (/^cch[a-f0-9]{8}$/i.test(targetRule)) {
    ...
    for (const habits of Object.values(cats)) {
      const match = habits.find(
        (h) => getRuleHash(h.rule) === targetRule.toLowerCase(),
      );
      if (match) {
    ...
    if (!found) {
      process.stderr.write(
        `  Error: habit hash ${rule} not found in habits.md\n`,
      );
      return 1;
    }
  }
  ...
  if (removed)
    process.stdout.write(
      c(DIM, "  Removed from active habits and preferences.md.\n"),
    );
...
export function cmdDiff(since?: number): number {
  ...
  if (!d) {
    process.stdout.write(
      c(DIM, "  Not enough history yet. Run a Claude Code session first.\n"),
    );
    return 0;
  }
  process.stdout.write(
    c(BOLD, "\n  Habits diff") +
      c(DIM, `  ${d.fromTs.slice(0, 19)} → ${d.toTs.slice(0, 19)}\n\n`),
  );
  ...
  if (d.added.length > 0) {
    ...
    for (const a of d.added)
      process.stdout.write(
        `    ${c(GREEN, "+")} [${a.category}] ${a.habit.rule}\n`,
      );
    process.stdout.write("\n");
  }
  ...
  if (d.changed.length > 0) {
    ...
    for (const ch of d.changed) {
      ...
      process.stdout.write(
        `    ${c(colour, arrow)} [${ch.category}] ${ch.rule}  ${ch.from.toFixed(2)} → ${c(BOLD, ch.to.toFixed(2))}\n`,
      );
    }
    process.stdout.write("\n");
  }
...
export function cmdExplain(query: string): number {
  if (!query) {
    process.stderr.write(
      "cc-habits explain: requires a habit rule (or substring).\n",
    );
    return 1;
  }
  ...
  process.stdout.write(
    c(
      DIM,
      `  Category: ${term(exp.category)}  ·  Confidence: ${(exp.confidence * 100).toFixed(0)}%  ·  Sessions: ${exp.sessions_seen}\n`,
    ),
  );
  process.stdout.write(
    c(DIM, `  ↑${exp.reinforcing}  ↓${exp.contradicting}\n`),
  );
  process.stdout.write("\n");
  if (exp.refs.length === 0) {
    process.stdout.write(
      c(
        DIM,
        "  No provenance recorded yet (habit predates v0.2 provenance tracking).\n\n",
      ),
    );
    return 0;
  }
  ...
  for (const ref of exp.refs) {
    process.stdout.write(
      `  ${c(CYAN, term(ref.file))}  ${c(DIM, term(ref.ts.slice(0, 19)))}  ${c(YELLOW, term(ref.decision))}\n`,
    );
    ...
    for (const ln of lines) {
      if (ln.startsWith("+"))
        ...
        process.stdout.write(`    ${c(RED, term(ln.slice(0, 80)))}\n`);
...
export async function cmdLint(
  ...
  if (!filePath) {
    process.stderr.write(
      "cc-habits lint: requires a file path.\n  Usage: cc-habits lint <file>\n",
    );
    return 1;
  }
  let findings;
  try {
    ...
    if (
      getConfigValue("provider") === "ollama" &&
      process.stdin.isTTY &&
      !asJson
    ) {
  ...
  process.stdout.write(
    c(
      BOLD,
      `  ${findings.length} violation${findings.length === 1 ? "" : "s"} in ${filePath}\n\n`,
    ),
  );
...
export async function cmdBootstrap(): Promise<number> {
  ...
  if (sessions.length === 0) {
    process.stdout.write(
      c(DIM, "  No developer tool sessions found for this project.\n"),
    );
    process.stdout.write(
      c(
        DIM,
        "  Start a session using Claude Code, Codex, Gemini, or Kimi, make some edits, then try again.\n",
      ),
    );
    return 0;
  }
  ...
  try {
    ...
    if (result.signalsExtracted === 0) {
      process.stdout.write(
        c(DIM, `  Sessions found but no edits to learn from.\n\n`),
      );
      return 0;
    }
    ...
    if (result.habitsLearned === 0 && result.habitsReinforced === 0) {
      process.stdout.write(
        c(
          DIM,
          "  No new patterns found (sessions may have already been processed).\n\n",
        ),
      );
      return 0;
    }
    ...
    if (result.habitsReinforced > 0) {
      process.stdout.write(
        `  ${c(GREEN, "✓")} Reinforced ${result.habitsReinforced} existing habit${result.habitsReinforced === 1 ? "" : "s"}\n`,
      );
    }
    process.stdout.write("\n");
    process.stdout.write(
      c(DIM, "  Run `cc-habits view` to see your habits.\n\n"),
    );
    return 0;
  } catch (e) {
...
export async function cmdImport(source: string): Promise<number> {
  if (!source) {
    process.stderr.write(
      "cc-habits import: requires a file path or https:// URL.\n  Usage: cc-habits import <file.md|https://...>\n",
    );
    return 1;
  }
  ...
  if (source.startsWith("http://")) {
    process.stderr.write(
      "cc-habits import: only https:// URLs are supported (not http://).\n",
    );
    return 1;
  }
  ...
  const memNote =
    result.memoriesImported !== undefined && result.memoriesImported > 0
      ? `, ${result.memoriesImported} memories imported`
      : "";
  process.stdout.write(
    `  imported: ${result.added} new habit${result.added === 1 ? "" : "s"}, ${result.merged} merged${memNote}\n`,
  );
  return 0;
}

// sync (Patch 1: portable AGENTS.md / Cursor / Cline emitter) ───────────────
const VALID_SYNC_TARGETS: SyncTarget[] = [
  "agents",
  "cursor",
  "copilot",
  "gemini",
...
export function cmdSync(rawTargets: string[], dir?: string): number {
  ...
  if (rawTargets.length === 0) {
    ...
    const invalid = rawTargets.filter(
      (t) => !VALID_SYNC_TARGETS.includes(t as SyncTarget),
    );
    if (invalid.length > 0) {
      process.stderr.write(
  ...
  try {
    ...
    if (result.skipped) {
      process.stdout.write(
        c(
          DIM,
          "  No active habits to sync yet. Keep coding, habits graduate after 2 sessions.\n",
        ),
      );
      return 0;
    }
...
export async function cmdMigrate(force = false): Promise<number> {
  ...
  if (result.migrated) {
    ...
    process.stdout.write(
      `  no migration needed or destination already populated.\n`,
    );
  }
  return 0;
}
...
// capture ──────────────────────────────────────────────────────────────────
export function cmdCapture(opts: {
  file: string;
  diff: string;
  session?: string;
  source?: string;
...
export async function cmdGitCapture(range?: string): Promise<number> {
  ...
  if (signalsCaptured > 0) {
    process.stdout.write(
      `  captured ${signalsCaptured} git commit signal${signalsCaptured === 1 ? "" : "s"} to the local capture log:\n`,
    );
    ...
    if (captured.length > SHOWN) {
      process.stdout.write(
        c(
          DIM,
          `    ... and ${captured.length - SHOWN} more (see \`cch log\`)\n`,
        ),
      );
    }
    ...
    if (
      getConfigValue("provider") === "ollama" &&
      isCloudOllamaModel(getConfigValue("ollama_model"))
    ) {
      process.stdout.write(
        c(
          YELLOW,
          `  These are stored locally, but your Ollama model ${getConfigValue("ollama_model")} runs on Ollama's cloud,\n  so \`cch learn\` will send redacted diffs off your machine.\n`,
        ),
      );
    } else {
      process.stdout.write(
        c(
          DIM,
          "  These stay on your machine. Nothing is sent until you run `cch learn` with a provider.\n",
        ),
      );
    }
    if (shouldTriggerGitLearn()) {
      process.stdout.write(
        `  git signal threshold met. Triggering automated learn...\n`,
      );
...
export function renderRepoScan(scan: RepoScanResult): void {
  if (!scan.scanned) {
    if (scan.reason === "already scanned") {
      process.stdout.write(
        c(
          DIM,
          "  This repo was already scanned. Re-run with `cch learn --repo`.\n",
        ),
      );
      ...
      process.stdout.write(
        c(
          DIM,
          "  Repo scan skipped: no AI provider configured. Add one: `cch init --provider anthropic`.\n",
        ),
      );
    } else {
      process.stdout.write(
        c(
          DIM,
          `  Repo scan skipped: ${scan.reason ?? "nothing to analyze"}.\n`,
        ),
      );
      if (scan.suggestion) {
        process.stdout.write(
          "  " + c(YELLOW, "→") + " " + c(DIM, scan.suggestion) + "\n",
        );
      }
    }
    return;
  ...
  if (learned === 0 && scan.habitsUpdated === 0 && memoriesUpdated === 0) {
    ...
    process.stdout.write(
      c(
        DIM,
        `  Scanned ${fileCount}; the model returned no habits this pass.\n`,
      ),
    );
  ...
  const bits: string[] = [];
  if (scan.habitsLearned > 0)
    bits.push(
      `${c(BOLD, String(scan.habitsLearned))} new habit${scan.habitsLearned === 1 ? "" : "s"}`,
    );
  if (scan.habitsUpdated > 0) bits.push(`${scan.habitsUpdated} reinforced`);
  if (scan.memoriesLearned > 0)
    bits.push(
      `${c(BOLD, String(scan.memoriesLearned))} memor${scan.memoriesLearned === 1 ? "y" : "ies"}`,
    );
  if (memoriesUpdated > 0)
    bits.push(
      `${memoriesUpdated} memor${memoriesUpdated === 1 ? "y" : "ies"} reinforced`,
    );
  process.stdout.write(
  ...
  if (scan.details) {
    ...
    if (
      scan.details.reinforcedMemories &&
      scan.details.reinforcedMemories.length > 0
    ) {
      process.stdout.write(
...
export async function cmdLearnRepo(
  opts: { force?: boolean; ctx?: StorageContext } = {},
): Promise<number> {
  const ctx = opts.ctx ?? repoStoreCtx();
  process.stdout.write(
    c(DIM, "  Scanning this repository into its .cch/ store...\n"),
  );
  try {
    const scan = await scanRepo({
...
export async function cmdLearnScoped(
  opts: { session?: string; since?: number } = {},
): Promise<number> {
  if (!process.stdin.isTTY) return cmdLearn(opts);
  const choice = await runSelectMenu("  Where should cc-habits learn into?", [
    {
      label: "This repo     .cch/ store, scoped to this repository",
      value: "repo",
    },
    {
      label: "This session  global store, applies everywhere",
      value: "session",
    },
...
export async function cmdLearn(
  opts: { session?: string; since?: number; interactive?: boolean } = {},
): Promise<number> {
  ...
  if (filtered.length < 3) {
    process.stdout.write(
      `  not enough signals in capture log (found ${filtered.length}). Falling back to repository scan...\n\n`,
    );
    return cmdLearnRepo({ force: true });
  }

  const { batch: capped, desc: batchDesc } = capBatch(
    filtered,
    byteBudgetFor(getConfigValue("provider")),
  );
  ...
  const sendBatch = capped.map((s) =>
    s.diff ? { ...s, diff: redact(s.diff) } : s,
  );

  // Live trace: each line fires at a real boundary in the work, never on a timer.
  const progress = steppedProgress();
  progress.done(
    `read ${batchDesc} edit${filtered.length === 1 ? "" : "s"} · redacted locally`,
  );
  ...
  if (deleted.length > 0)
    progress.done(`forgetting ${deleted.length} you deleted`);

  const decayed = applyDecay(cats);

  const sessionId =
    opts.session || `learn-${new Date().toISOString().slice(0, 10)}`;
  ...
  const fallbackLanguage =
    sessionLanguages.length === 1 ? sessionLanguages[0] : undefined;
  ...
  const creates = updates.filter(
    (u) => (u.decision ?? "").toLowerCase() === "create",
  );
  ...
  if (memoriesEnabled()) {
    try {
      ...
      const candidates = await progress.spin(
        privacyNote
          ? `noticing corrections you made · ${privacyNote}`
          : "noticing corrections you made",
        () => extractMemoryCandidates(sendBatch, memoriesMd),
        { motion: "distill" },
      );
      memoryCandidatesCount = applyMemoryUpdates(
        candidates,
        undefined,
        addedMemories,
        updatedMemories,
      ...
      if (hint)
        process.stdout.write(
          c(YELLOW, `  memory extraction skipped: ${hint}\n`),
        );
  ...
  const summary = formatStopSummary({
    ...
    learningCount: Object.values(cats)
      .flat()
      .filter((h) => (h.sessions_seen ?? 1) < 2).length,
